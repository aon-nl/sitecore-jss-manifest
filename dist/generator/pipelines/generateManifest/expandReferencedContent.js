...
var createIdMapping_1 = require("../../createIdMapping");
...
function expandReferencedContent(manifest) {
    ...
    idMap.usages.reverse().forEach(function (idReference) {
        ...
        if (targetType === 'rendering') {
            ...
            if (idReference.reference.copy &&
                idReference.reference.dataSource &&
                idReference.reference.dataSource.name) {
                expandedRef.dataSource.name = idReference.reference.dataSource.name.replace('undefined', // hack around default datasource namer using the componentName, which isn't defined when its an ID reference
            ...
            if (idReference.reference.copy &&
                ...
                expandedRef.dataSource.displayName = idReference.reference.dataSource.displayName.replace('undefined', // hack around default datasource namer using the componentName, which isn't defined when its an ID reference
                expandedRef.dataSource.template);
            }
        }
        if (sourceType !== 'rendering' && targetType === 'rendering') {
            ...
            if (expandedRef.params) {
                ...
                expandedRef.renderingParams = Object.keys(expandedRef.params).reduce(function (result, current) {
