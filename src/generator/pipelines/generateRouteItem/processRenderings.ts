...
const generateRenderingParams = (component: any, rendering: any) => {
  ...
  if (invalidParams.length > 0) {
    ...
    // tslint:disable-next-line:max-line-length no-string-throw
    console.warn(chalk.red(`Param(s) ${invalidParamsString} defined on an instance of component ${component?.name} was not defined on the component definition. Add it to the manifest component definition to use it. Valid params: ${validParams}. Instance definition: ${JSON.stringify(rendering, null, 2)}`));
  }
...
const generateFields = (
  ...
  if (component) {
    // tslint:disable-next-line:max-line-length
    const handleError = (fieldName: string) => { console.warn(chalk.red(`${dataSourceItem.name} route datasource defined data for '${fieldName}' on component ${component.name}. This field is not defined on this component. It may be a typo, or the field may need to be added to the component definition.`)) };
...
const processRendering = (rendering: any, index: number, context: GenerateRouteItemPipelineArgs) => {
  ...
  if (!component && rendering.componentName) {
    // tslint:disable-next-line:max-line-length
    console.warn(chalk.red(`The component '${rendering.componentName}' used on route '${context.route.name}' was not defined in the manifest. Please define this component with 'manifest.addComponent()', or change the name to an existing component name.`)
    );
  }
